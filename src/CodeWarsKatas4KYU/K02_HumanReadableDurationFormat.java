package CodeWarsKatas4KYU;

/* <------------------------------------------------
    DESCRIPTION:

    Your task in order to complete this Kata is to write a function which formats
    a duration, given as a number of seconds, in a human-friendly way.

    The function must accept a non-negative integer.
    If it is zero, it just returns "now".
    Otherwise, the duration is expressed as a combination of years, days, hours, minutes and seconds.

    Examples:
        * For seconds = 62, your function should return "1 minute and 2 seconds"
        * For seconds = 3662, your function should return "1 hour, 1 minute and 2 seconds"

    For the purpose of this Kata, a year is 365 days and a day is 24 hours.

    Note that spaces are important.

    Detailed rules:
        The resulting expression is made of components like 4 seconds, 1 year, etc.
        In general, a positive integer and one of the valid units of time, separated by a space.
        The unit of time is used in plural if the integer is greater than 1.

        The components are separated by a comma and a space (", ").
        Except the last component, which is separated by " and ", just like it would be written in English.

        A more significant units of time will occur before than the least significant one.
        Therefore, 1 second and 1 year is not correct, but 1 year and 1 second is.

        Different components have different unit of times.
        So there is not repeated units like in 5 seconds and 1 second.

        A component will not appear at all if its value happens to be zero.
        Hence, 1 minute and 0 seconds is not valid, but it should be just 1 minute.

        A unit of time must be used "as much as possible".
        It means that the function should not return 61 seconds, but 1 minute and 1 second instead.
        Formally, the duration specified by of a component must not be
        greater than any valid more significant unit of time.
---------------------------------------------------> */

public class K02_HumanReadableDurationFormat {

    public static void main(String[] args) {

        System.out.println(formatDuration(3600));
        System.out.println(formatDuration(359999));
        System.out.println(formatDuration(32_536_999));
        System.out.println(formatDuration(31_586_599));
        System.out.println(formatDuration(31_586_419));
    }

    public static String formatDuration(int time) {

        int seconds = time  % 60, minutes = (time / 60) % 60, hours = (time / 3600) % 24, days = (time / 86400) % 365, years = time / 31_536_000;

        StringBuilder sb = new StringBuilder();

        if (years == 1) sb.append("1 year");
        if (years > 1) sb.append(String.format("%d years", years));

        if (years > 0) {
            if (days == 1) sb.append(", 1 day");
            else if (days > 1) sb.append(String.format(", %d days", days));
        } else if (years == 0) {
            if (days == 1) sb.append("1 day");
            else if (days > 1) sb.append(String.format("%d days", days));
        }

        if (years > 0 || days > 0) {
            if (hours == 1) sb.append(", 1 hour");
            else if (hours > 1) sb.append(String.format(", %d hours", hours));
        } else if (years == 0 && days == 0) {
            if (hours == 1) sb.append("1 hour");
            else if (hours > 1) sb.append(String.format("%d hours", hours));
        }

        if (years > 0 || days > 0 || hours > 0) {
            if (minutes == 1) sb.append(", 1 minute");
            else if (minutes > 1) sb.append(String.format(", %d minutes", minutes));
        } else if (years == 0 && days == 0 && hours == 0) {
            if (minutes == 1) sb.append("1 minute");
            else if (minutes > 1) sb.append(String.format("%d minutes", minutes));
        }

        if (years > 0 || days > 0 || hours > 0 || minutes > 0) {
            if (seconds == 1) sb.append(", 1 second");
            else if (seconds > 1) sb.append(String.format(", %d seconds", seconds));
        } else if (years == 0 && days == 0 && hours == 0 && minutes == 0) {
            if (seconds == 1) sb.append("1 second");
            else if (seconds > 1) sb.append(String.format("%d seconds", seconds));
        }

        String result = sb.toString();
        if (result.contains(",")) result = result.substring(0, result.lastIndexOf(",")) + " and" + result.substring(result.lastIndexOf(",") + 1);

        return result;
    }
}
